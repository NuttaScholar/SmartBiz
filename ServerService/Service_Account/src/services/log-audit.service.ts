...
export default class LogAuditService {
  ...
  async getById(id: string): Promise<LogAuditView> {
    if (!Types.ObjectId.isValid(id)) {
      throw invalidInput("Invalid log audit id");
    }
  ...
  async query(query: LogAuditQuery): Promise<LogAuditQueryResult> {
    ...
    if (query.transactionId) {
      if (!Types.ObjectId.isValid(query.transactionId)) {
        throw invalidInput("Invalid transactionId");
      }
    ...
    addNumberRange(
      filter,
      "wallet.beforeAmount",
      query.minBeforeAmount,
      query.maxBeforeAmount,
    );
    addNumberRange(
      filter,
      "wallet.afterAmount",
      query.minAfterAmount,
      query.maxAfterAmount,
    );
...
function toView(log: any): LogAuditView {
  return {
    ...
    transactionBefore: log.transactionBefore,
    transactionAfter: log.transactionAfter,
    wallet: log.wallet,
...
function addNumberRange(
  ...
  field: "wallet.beforeAmount" | "wallet.afterAmount",
  min?: string,
  max?: string,
) {
